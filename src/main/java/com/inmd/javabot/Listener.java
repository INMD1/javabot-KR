...
public class Listener extends ListenerAdapter
{
    ...
    public void onReady(ReadyEvent event) 
    {
        if(event.getJDA().getGuilds().isEmpty())
        {
            ...
            log.warn(event.getJDA().getInviteUrl(javabot.RECOMMENDED_PERMS));
        }
        credit(event.getJDA());
    ...
    private void credit(JDA jda)
    {
        ...
        jda.getTextChannelById(119222314964353025L)
                .sendMessage("이 계정은 javabot-KR을 실행 중입니다. 이 서버에 봇 클론을 나열하지 마십시오, <@"+bot.getConfig().getOwnerId()+">.").complete();
        dbots.leave().queue();
    }
}
