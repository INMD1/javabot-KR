 ...
 */
package com.inmd.javabot.utils;
...
public class OtherUtil
{
    public final static String NEW_VERSION_AVAILABLE = "JAVABot-KR의 새로운 버전이 있습니다!\n"
                    + "현재 버전 : %s\n"
                    + "새로운 버전: %s\n\n"
                    + "최신 릴리스를 얻으려면 https://github.com/INMD1/javabot-KR/releases/latest 를 방문하십시오.";
    ...
    public static Path getPath(String path)
    {
        ...
        if(path.toLowerCase().startsWith(WINDOWS_INVALID_PATH))
        {
            ...
            try
            {
                path = new File(javabot.class.getProtectionDomain().getCodeSource().getLocation().toURI()).getParentFile().getPath() + File.separator + filename;
            }
    ...
    public static Activity parseGame(String game)
    {
        ...
        if(lower.startsWith("listening to"))
        	 return Activity.playing(makeNonEmpty(game.substring(12).trim()));
        if(lower.startsWith("listening"))
        	 return Activity.playing(makeNonEmpty(game.substring(9).trim()));
        if(lower.startsWith("watching"))
        	 return Activity.playing(makeNonEmpty(game.substring(8).trim()));
