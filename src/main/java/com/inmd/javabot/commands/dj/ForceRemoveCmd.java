...
public class ForceRemoveCmd extends DJCommand
{
    ...
    private void removeAllEntries(User target, CommandEvent event)
    {
        ...
        if (count == 0)
        {
            event.replyWarning("**"+target.getName()+"** 대기열에 노래가 없습니다!");
        }
        else
        {
            event.replySuccess("성공적으로 제거되었습니다. `"+count+"` 의 항목 **"+target.getName()+"**#"+target.getDiscriminator()+".");
        }
    }
}
